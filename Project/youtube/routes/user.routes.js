...
router.post("/update-profile", checkAuth, async (req, res) => {
    try {
        ...
        if (req.files && req.files.logoUrl.tempFilePath) {
...
router.post("/subscribe", checkAuth, async (req, res) => {
    try {
        const { channelId } = req.body // userId == current user, channelId = user to subscribe
        if (req.user._id === channelId) {
            return res.status(400).json({ Message: "you can not subscribe your channel" });
        }
        ...
        const subscribedUser = await User.findByIdAndUpdate(channelId, {
            $inc: { subscribedChannels: channelId }
        })
